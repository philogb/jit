    ...
    var config = {
        ...
        Events: {
            ...
            ((i < clade.length) & (i < 30)); i++) {
                (function (index) {
                    var node = clade[i],
                        sampleid, result, species;
                    temp = [];
                    species = node.name.match(/[^\d]*/);
                    // occurences map
                    result = species ? "<a class='thumbImage1' rel='"+species[0]+i+"' title='"+species[0] + ' occurrences Map' 
                                              +"' href='http://biocache.ala.org.au/ws/occurrences/static?q="
                                                   +species[0].replace(/\s+/g,'+')+"'><id='thumb1'><div class='forward'></div></a>" : '';
                                     temp.push ( result ); 

                                     // density map
                                     result = species ? "<a class='thumbImage1' rel='"
                                         +species[0]+i+"' title='"+species[0] + ' density Map' 
                                     ...
                                     // ALA Spatial portal 
                                    result = species ? "<a class='thumbImage1' rel='"
                                         +species[0]+i+"' title='"+species[0]+ ' in ALA spatial portal'
                                         +"' id='"+species[0].replace(/\s+/g,'ALA_iMAP')+index
                                         +"'class='thumbImage1' rel='"+species[0]+index+"' title='' href='' id='thumb1'></a>" : '';
                                     temp.push ( result );

                                     // Australian plant image index
                                     result = species ? "<a class='thumbImage1' rel='"
                                              +species[0]+i+"' title='"+ 'images of ' + species[0]+' in Aust. plant Image Index'
                                               +"' href='http://www.anbg.gov.au/cgi-bin/apiiGenus?genus="+species[0].replace(/\s+/g,'&species=')
                                                +"'><id='thumb1'><div class='forward'></div> </a>" : '';
                                     temp.push ( result );

                                     // EOL images
                                     result = species ? "<a class='thumbImage1' rel='"
                                              +species[0]+i+"' title='"+ 'images of ' + species[0]+' in EOL' 
                                               +"' href='http://eol.org/search/show?q="+species[0].replace(/\s+/g,'+') + "&type[]=taxon_concept&type[]=image&commit=Filter"
                                                +"'><id='thumb1'><div class='forward'></div> </a>" : '';
                                     temp.push ( result );

                                     // Paul: also a link to the ALA representative image page via LSID  call is http://bie.ala.org.au/species/image/large/+lsid
         
                                     // NSL - australian national species list
                                    result = species ? "<a class='thumbImage1' rel='"
                                             +species[0]+i+"' title='"+ 'about '+species[0]+' in NSL nomenclator ' +"' href='http://biodiversity.org.au/name/"
                                             +species[0].replace(/\s+/g,'+')+ "'>\<id='thumb1'><div class='forward'></div> </a>"  : '';
                                     temp.push ( result );


                                     // ALA BIE Species pages
                                     result = species ? "<a class='thumbImage1' rel='"+species[0]+i+"' title='"
                                            +species[0]+' ALA species page' +"' href='http://bie.ala.org.au/species/"+species[0].replace(/\s+/g,'+')+ "'>"+species[0]+"</a>"  : '';
                                     temp.push ( result );
                                    ...
                                    $.ajax({url:'/bin/rest/OpenLayersPlugin/getJSON?url=http://bie.ala.org.au/ws/guid/+'+species[0].replace(/\s+/g,'+'),success:function( data ) { 
                                     if ( data ) { 
                                                   $('a#'+species[0].replace(/\s+/g,'ALA_iMAP')+index).attr('href','http://spatial.ala.org.au/?q=lsid:'+data[0].identifier+'#portal');
                                                   $('a#'+species[0].replace(/\s+/g,'ALA_iMAP')+index).html( "<div class='forward'></div>" );
                                                  } else {
                                                     $('a#'+species[0].replace(/\s+/g,'ALA_iMAP')+index).attr('rel','');
                                                     }
                                                }
                                       });
                                   })( i );     
                                 ...
                                 return '<table style="border: none">'+result+'</table>';
                      }, //presentClade 

          onPresentClade:function ( ) {
                  $('.thumbImage1').colorbox({iframe:true,width:'80%',height:'80%'});
           }, // onPresentClade
          Tips:{
            enable:true,
            onShow:function( div, node){
