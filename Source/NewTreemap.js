...
TM.Base = {  
  ...
  initialize: function(controller) {
    ...
    this.controller = this.config = 
      $merge(Options("Canvas", "Node", "Edge", "Fx", "Controller", "Tips", "NodeStyles"),
        config, controller);
  ...
  out: function() {
   var GUtil = Graph.Util, parents = GUtil.getParents(this.graph.getNode(this.root));
   if(parents.length > 0) {
...
TM.Plot.NodeTypes = new Class({
  ...
  'rectangle': {
    'render': function(node, canvas) {
      ...
      if(leaf) {
        var lg = ctx.createLinearGradient(posx, posy, posx + width - offst, posy + height - offst);
        lg.addColorStop(0, '#555');
        lg.addColorStop(1, '#ccc');
        ctx.fillStyle = lg;
        ctx.fillRect(posx, posy, width - offst, height - offst);
      } else {
        var lg = ctx.createLinearGradient(posx, posy, posx + width - offst, posy + titleHeight);
        lg.addColorStop(0, '#111');
        lg.addColorStop(1, '#444');
        ctx.fillStyle = lg;
        ctx.fillRect(pos.x + offst/2, pos.y + offst/2, width - offst, titleHeight);
      }
    },
