  ...
  window.tm = new $jit.TM.Voronoi({
    ...
    Tips: {
      ...
      onShow: function(tip, node, isLeaf, domElement) {
        ...
        if (data.image) {
          html += "<img width=\"139\" src=\"" + data.image + "\" class=\"album\" />";
        }
        if (data.$area) {
          html += "<b>weight/area: " + data.$area.toFixed(2) + '/' + G.area(data.$vertices).toFixed(2) + '<br/>=' + (data.$area / G.area(data.$vertices) * 10000).toFixed(2) + "</b><br/>"
        }
        tip.innerHTML = html;
      }
    },
