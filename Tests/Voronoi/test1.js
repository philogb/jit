...
function draw() {
  ...
  var pos = $jit.Voronoi.voronoiFortune(ps, polygon);
  var cents = pos.map(function(po, ind) { if (!po.length) return ps[ind]; return $jit.util.centroid(po); });
  var cpos = $jit.Voronoi.voronoiFortune(cents, polygon);
	...
	$jit.util.each(pos, function(p, ind) {
      ...
      $jit.util.each(ps[ind].parts, function(part) {
        ...
        dp(part);
        ctx.beginPath();
        var cent = $jit.util.centroid(pos[part.ind]);
    ...
    ctx.fillStyle = "#000";
    ctx.fillText(ind + ":" +(ps[ind].area / $jit.util.area(p)).toFixed(2), ps[ind].x, ps[ind].y);
	});
...
function stroke() {
  ...
  ps = [];
  for (var i = 0; i < 10; i++) {
    var c = $jit.util.randPointInPolygon(polygon);
    if (!$jit.util.pointInPolygon([c], polygon)[0]) {
  ...
  for (var i = 0; i < 100; i++){
    var polygons = $jit.Voronoi.voronoiFortune(ps, polygon);
    ps = $jit.util.map(polygons, function(p, ind) {
      var po = $jit.util.centroid(p); //$C(ps[ind].x, ps[ind].y);
...
function presure (sites, bound) {  
  ...
  tw = $jit.util.area(bound) / tw;
  $jit.util.each(sites, function(s) {
    s.area *= tw;
  });
  
  var polygons = $jit.Voronoi.voronoiFortune(sites, bound);
  var presure = polygons.map(function(p) {
    return p.area / ($jit.util.area(p) + 1e-10);
  });

  polygons = $jit.Voronoi.voronoiFortune(sites, bound);
  sites = $jit.util.map(polygons, function(p, ind) {
    ...
    for (var i = 0; i < p.length; i++) {
      var targetPresure = (p[i].attached) ? presure[p[i].attached[0]] : 1,
          start = p[i],
          stop = p[i + 1] || p[0],
          pr = (presure[ind] - targetPresure), 
          dx = stop.x - start.x,
          dy = stop.y - start.y;
...
function init() {
  ...
  console.log($jit.util.convexCut([new c(0,0), new c(1,2), new c(2,0)], [new c(0,1), new c(2,1)]));
  
  console.log($jit.Voronoi.voronoiFortune(
    [[0,0],[1,1]], [new c(0, 0), new c(0, 1), new c(1, 1), new c(1, 0)]));
  console.log($jit.util.convexCut([new c(0, 0), new c(1, 2), new c(2, 0)],[new c(0, 1), new c(2, 1)]));
  console.log($jit.util.convexCut([new c(0, 0), new c(1, 2), new c(2, 0)],[new c(2, 1), new c(0, 1)]));
  console.log($jit.util.convexCut([new c(0, 0), new c(1, 2), new c(2, 0)],[new c(2, 1), new c(0, 0)]));
  console.log($jit.util.convexCut([new c(1, 0), new c(2, 0), new c(1, 2),[0.5,1]],[new c(1, 0), new c(2, 0)]));
  
  console.log($jit.util.convexIntersect(
    [[0,0],[2,0],[1,2]],
    [[0,2],[1,0],[2,0]]));
